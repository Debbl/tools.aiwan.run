...
const markdownZh = `
# 🔗 URL 查询参数生成器 - 在线 URL 构建工具

## 📖 项目简介

URL 查询参数生成器是一个专业的在线工具，帮助您快速构建和修改 URL 查询参数。无论是 API 开发、网页链接构建还是参数测试，这个工具都能为您提供便捷、准确的 URL 参数管理服务。

## ✨ 主要功能

### 🎯 核心特性
- **URL 解析**: 自动解析现有 URL 的查询参数
- **参数编辑**: 支持添加、修改、删除查询参数
- **实时预览**: 实时显示构建的 URL 结果
- **一键打开**: 直接在新窗口中打开生成的 URL
- **响应式设计**: 完美适配桌面端和移动端设备

### 🛠️ 技术特点
- **高性能**: 基于 Next.js 15 构建，提供极速处理体验
...
### 基本操作
1. **输入 URL**: 在输入框中输入要处理的 URL
2. **解析参数**: 点击"解析"按钮自动提取查询参数
3. **编辑参数**: 添加、修改或删除查询参数
4. **生成 URL**: 系统会自动生成新的 URL
5. **打开链接**: 点击"获取"按钮在新窗口中打开

### 高级功能
- **参数管理**: 支持动态添加和删除参数
- **URL 验证**: 自动验证 URL 格式的正确性
- **历史记录**: 保存常用的 URL 参数组合
- **批量处理**: 支持批量处理多个 URL
...
- **URL API**: 使用浏览器原生 URL API

### 性能优化
- **实时处理**: 使用 useMemo 优化 URL 处理性能
- **代码分割**: 按需加载组件
- **缓存策略**: 智能的静态资源缓存
- **PWA 支持**: 离线访问和安装功能
...
### 关键词覆盖
- url 查询参数生成器, 在线 url 构建工具, url 参数编辑器
- url 参数生成器, 查询参数工具, url 构建器
- 在线 url 工具, url 参数管理, url 链接生成
- url 查询参数, 在线 url 编辑器
...
> 💡 **小贴士**: URL 查询参数生成器是 Web 开发和 API 测试的重要工具！
`

const markdownEn = `
# 🔗 URL Query Parameter Generator - Online URL Builder Tool
...
> 💡 **Tip**: URL Query Parameter Generator is an important tool for web development and API testing!
`
